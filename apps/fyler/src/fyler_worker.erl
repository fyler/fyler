...
handle_info(download, State) ->
  self() ! {error, {cannot_download_non_s3_file}},
  {noreply, State};
...
handle_info({process_complete, Stats}, #state{task = #task{file = #file{bucket = Bucket, dir = Dir, target_dir = TargetDir}, acl = Acl}} = State) ->
...
handle_info(abort_task, #state{process = Pid}=State) ->
  erlang:exit(Pid, abort),
  {noreply, State};

handle_info(_Info, State) ->
  {stop, {error, unknown_info, State}}.

...
handle_event(#fevent{type = complete, task = #task{id=TaskId, file = #file{url = Url}, type = Type} = Task, stats = #job_stats{time_spent = Time, download_time = DTime} = Stats}, State) ->
  ?D({task_complete, Type, Url, {time,Time},{download_time,DTime}}),

  ets:delete(?T_LIVE_STATS,TaskId),

  fyler_server:save_task_stats(Stats),
  fyler_server:send_response(Task,Stats,success),
  {ok, State};

handle_event(#fevent{type = failed, task = #task{id=TaskId, file = #file{url = Url}, type = Type} = Task, error = Error, stats = Stats}, State) ->

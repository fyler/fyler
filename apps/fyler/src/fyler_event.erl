...
-export([task_completed/2, task_started/1,  task_failed/2, pool_enabled/0,pool_disabled/0]).
...
task_started(Task) ->
  gen_event:notify({global, ?MODULE}, #fevent{type = started, node=node(), task = Task}).
